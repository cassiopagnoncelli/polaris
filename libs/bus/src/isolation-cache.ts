/**
 * TTL+LRU cache for stream-family isolation lookups.
 *
 * The resolver in `stream-family.ts` is consulted on every publish and
 ...
 * `stream-family.ts` so the resolver does not know whether it is talking
 * to PostgreSQL or the cache.
 *
 * @see docs/architecture/03-rabbitmq-streams.md "Stream Families"
 * @see docs/architecture/03-rabbitmq-streams.md "Stream Isolation Triggers"
 * @see docs/implementation/tasks/P11-008-topic-isolation.md
 */

import type { IsolationLookup } from "./stream-family.js";
import type { CanonicalStreamFamily } from "./streams.js";

/**
 * Extended async lookup contract that carries the environment scope.
 *
 * `IsolationLookup` in `stream-family.ts` predates the environment-scoped
 ...
 * cache through {@link StreamIsolationCache.isIsolatedScoped} directly.
 */
...
export interface StreamIsolationCacheOptions {
 ...
 * a colon (the `dedicatedStreamFamily` helper uses a single dot, so no
 * collision is possible with the family + project_id concatenation
 * either).
 */
function cacheKey(family: CanonicalStreamFamily, projectId: string, environment: string): string {
 ...
 * `IsolationLookup` contract by calling {@link StreamIsolationCache.forEnvironment}
 * with the environment they operate in.
 */
export class StreamIsolationCache implements ScopedIsolationLookup {
  readonly #reader: ScopedIsolationLookup;
  readonly #maxEntries: number;
  readonly #ttlMs: number;
  readonly #now: () => number;
  ...
  constructor(options: StreamIsolationCacheOptions) {
    this.#reader = options.reader;
    this.#maxEntries = options.maxEntries ?? 1024;
    this.#ttlMs = options.ttlMs ?? 60_000;
    this.#now = options.now ?? Date.now;
    if (this.#maxEntries < 1) {
      throw new RangeError("StreamIsolationCache.maxEntries must be >= 1");
    }
    if (this.#ttlMs < 0) {
      throw new RangeError("StreamIsolationCache.ttlMs must be >= 0");
    }
  }
...
export class InMemoryScopedIsolationLookup implements ScopedIsolationLookup {
  ...
  add(family: CanonicalStreamFamily, projectId: string, environment: string): void {
    this.#isolated.add(cacheKey(family, projectId, environment));
  }

  /** Remove a triple. Subsequent lookups return `false`. */
  remove(family: CanonicalStreamFamily, projectId: string, environment: string): void {
