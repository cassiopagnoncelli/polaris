// Refuse to start a sample when the machine has no file-watch capacity left.
//
...
//
// The fifteenth watcher is this sample. What a developer sees when it loses is
// not "out of watches": libuv reports the failed `FSEventStreamStart` as
// EMFILE, Watchpack reads the dead watcher as a deleted `.next/dev`, and
// Next.js restarts to recover — into the same failure, forever. Nothing in
// that loop names the dev stack, so the search starts in the wrong repo. It
// reads like the sample broke, and the last thing that touched the sample gets
// the blame — usually whatever install ran most recently.
//
...
// needs none of this. It exists because the samples live in the monorepo whose
// dev stack starves them.
  ...
  for (let round = 0; round < ROUNDS; round++) {
    ...
    console.error(
      `\nThis machine is out of file-watch capacity (${failure}).\n\n` +
        ...
        "  per service:\n\n" +
        "    make dev-stop\n\n" +
        "  The samples only need the ingester, so start the small stack instead\n" +
