/**
 * `@polaris/destination-host` — the bootstrap a vendor consumer's `app.ts`
 * used to write for itself.
 *
 * A separate package from `@polaris/shared-destinations` on purpose. That
 ...
 * `@polaris/shared-project-config`. Putting a Fastify bootstrap, a project
 ...
 * So the layering is:
 *
 *   shared-destinations   the runtime. Delivery, gate, retry, breaker.
 *   destination-host      the process around it. Postgres, AMQP, Fastify,
 *                         shutdown ordering.
