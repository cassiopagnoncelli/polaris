/**
 * Bootstrap tests for identity-resolver v1 — processor run registration.
 *
...
import { buildIdentityResolverApp } from "../src/app.js";
import type { IdentityResolverRuntimeConfig } from "../src/config.js";
import { InMemoryIdentityLinkRepository } from "../src/repository.js";

const TEST_CONFIG: IdentityResolverRuntimeConfig = {
  service: {
  ...
  rabbitmq: {
    ...
    clientId: "identity-resolver-test",
    tls: false,
    heartbeatSeconds: 30,
  ...
  resolver: {
    consumerGroup: "polaris-identity-resolver-v1-test",
  },
};
...
async function build(runs: InMemoryProcessorRunRepository, installShutdown = false) {
  return buildIdentityResolverApp({
    config: TEST_CONFIG,
    installShutdown,
...
describe("buildIdentityResolverApp run registration", () => {
...
describe("buildIdentityResolverApp activation gate", () => {
  it("threads the gate into the runtime, so a disable reaches the message path", async () => {
    ...
    const runs = new InMemoryProcessorRunRepository();
    const result = await buildIdentityResolverApp({
      config: TEST_CONFIG,
