/**
 * Bootstrap tests for geoip-enricher v1 — processor run registration.
 *
...
import { buildGeoipEnricherApp } from "../src/app.js";
import type { GeoipEnricherRuntimeConfig } from "../src/config.js";

const TEST_CONFIG: GeoipEnricherRuntimeConfig = {
  service: {
    serviceName: "geoip-enricher",
  ...
  enricher: {
    consumerGroup: "polaris-geoip-enricher-v1-test",
  },
};
...
describe("buildGeoipEnricherApp run registration", () => {
  it("registers a processor_runs row for this process", async () => {
    ...
    try {
      ...
      expect(row?.processor_name).toBe("geoip-enricher");
...
describe("buildGeoipEnricherApp activation gate", () => {
  it("threads the gate into the runtime, so a disable reaches the message path", async () => {
    ...
    const runs = new InMemoryProcessorRunRepository();
    const result = await buildGeoipEnricherApp({
      config: TEST_CONFIG,
