/**
 * P8-006: cross-processor manifest validation for geoip-enricher v1.
 *
...
describe("geoip-enricher v1 manifest", () => {
  ...
  it("carries the expected identity fields", () => {
    expect(loaded.manifest.name).toBe("geoip-enricher");
  ...
  it("consumes raw.events and emits enriched.events", () => {
    expect(loaded.manifest.inputs).toEqual([{ family: "raw.events", schema_versions: "*" }]);
    expect(loaded.manifest.outputs).toEqual([{ family: "enriched.events", schema_versions: [1] }]);
  });

  it("declares no durable state stores (stateless beyond the lookup backend)", () => {
