/**
 * Polaris attribution-engine v1 binary entry point.
 *
 ...
 * through the `buildAttributionEngineApp` factory, starts the HTTP
...
import { buildAttributionEngineApp } from "./app.js";
import { loadAttributionEngineConfig } from "./config.js";

async function main(): Promise<void> {
  const config = loadAttributionEngineConfig();
  const { bootstrap } = await buildAttributionEngineApp({ config });
  ...
  try {
    ...
    logger.info(
      ...
      "attribution-engine listening",
    );
  } catch (err) {
    logger.error({ err }, "attribution-engine failed to start");
    process.exit(1);
  }
}
