 ...
 *
 * Follows the seam `processors/identity-resolver/v1/src/repository.ts`
...
export function parseTouchpointStoreKey(store_key: string): TouchpointStoreKeyParts | undefined {
  ...
  return {
    project_id,
    environment,
    primary_identifier_kind: tail.slice(0, colon) as PrimaryIdentifierKind,
    primary_identifier_value: tail.slice(colon + 1),
  };
}
