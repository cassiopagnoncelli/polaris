...
function defaultStore(env: NodeJS.ProcessEnv): DestinationsEnableReplayStore {
  ...
  return {
    ...
    enableReplayWithAudit: async (id, reason, now, audit) => {
      ...
      const outcome = await setDestinationReplayOptInWithAudit(
        handle.db,
        { row, enabled: true, reason },
        {
