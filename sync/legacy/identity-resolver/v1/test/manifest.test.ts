/**
 * P8-006: cross-processor manifest validation for identity-resolver v1.
 *
 * Loads the on-disk `processor.manifest.yaml` and asserts the field set
 * that every released v1 processor MUST carry. See
 * `docs/development/processor-manifests.md` for the schema and the
 * semantic-immutability rule that drives these assertions.
 */
...
describe("identity-resolver v1 manifest", () => {
  ...
  it("carries the expected identity fields", () => {
    expect(loaded.manifest.name).toBe("identity-resolver");
    expect(loaded.manifest.version).toBe("v1");
    expect(loaded.manifest.owner).toBe("platform-identity");
  });
  ...
  it("consumes raw.events and emits identity.events", () => {
    expect(loaded.manifest.inputs).toEqual([{ family: "raw.events", schema_versions: "*" }]);
    expect(loaded.manifest.outputs).toEqual([{ family: "identity.events", schema_versions: [1] }]);
  });

  it("declares the durable PostgreSQL state store", () => {
    expect(loaded.manifest.state_stores).toContain("postgres:identity_links");
  });
