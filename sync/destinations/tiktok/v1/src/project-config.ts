/**
 * tiktok's per-`(project, environment)` configuration contract.
 *
 ...
 * default, which is exactly what `POLARIS_TIKTOK_*` meant before the cutover.
 * A schema default would look tidier and would silently override an operator
 * who tuned those variables for their deployment.
 *
 ...
 *
 * What is NOT here: the credential. `{access_token, pixel_id}` lives in
 * `destinations.secret_value` on the destination row, because it belongs to
 * one instance rather than to the project — two TikTok pixels for one project
...
export const PROJECT_CONFIG_NAMESPACE = "tiktok";
...
export const projectConfigSchema = z.object({
  ...
  /** Events API host. Overridden for staging or test endpoints. */
  api_host: z.string().min(1).optional(),
  /** Per-attempt HTTP timeout, milliseconds. */
  request_timeout_ms: positiveIntSchema.optional(),
});

export type TikTokProjectConfig = z.infer<typeof projectConfigSchema>;
 ...
 */
export function parseTikTokProjectConfig(
