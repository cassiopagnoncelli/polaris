/**
 * Polaris tiktok v1 binary entry point.
 *
 ...
 * client + destination consumer runtime through `buildTikTokApp`,
...
import { buildTikTokApp } from "./app.js";
import { loadTikTokConfig } from "./config.js";

async function main(): Promise<void> {
  const config = loadTikTokConfig();
  const { bootstrap } = await buildTikTokApp({ config });
  ...
  try {
    ...
    logger.error({ err }, "tiktok failed to start");
    process.exit(1);
  }
}
