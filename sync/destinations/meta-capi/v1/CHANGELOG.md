...
- Depends on the additive `EnvelopeAppContext` extension to `@polaris/delivery-normalize`'s `FlatContext`.
...
- First real-vendor consumer of the destination runtime (`@polaris/delivery-destinations`, P9-001).
...
- Secret shape: JSON `{ pixel_id, access_token, test_event_code? }` resolved through `@polaris/runtime-secrets`. The plaintext access token never lands in PostgreSQL or audit_records.
- Error class mapping:
  - HTTP 2xx → accepted
