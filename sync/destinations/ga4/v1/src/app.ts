/**
 * Service bootstrap for ga4 v1.
 *
 * Mirrors `sync/destinations/tiktok/v1/src/app.ts`:
 *
 ...
 *   4. Build the `DestinationConsumer` runtime from the ga4
...
export async function buildGa4App(options: BuildAppOptions): Promise<BuiltGa4App> {
  ...
  void projectConfigStore.start().catch((err: unknown) => {
    ...
    logger.warn(
      { component: "ga4.project-config", err },
 ...
 * `sync/destinations/tiktok/v1/src/app.ts` and `apps/ingester-api/src/app.ts`:
 * user/password URL-encoded; sslmode explicit via URLSearchParams
 * ('require' or 'disable').
 */
function buildDb(
  config: Ga4RuntimeConfig,
...
function buildConsumer(
  ...
  const consumer = createPolarisConsumer({
    ...
    consumerVersion: CONSUMER_VERSION,
    poison: { component: "ga4", producer },
    groupName: config.ga4.consumerGroup,
    checkpoints,
  });
