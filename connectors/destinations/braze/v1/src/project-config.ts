/**
 * braze's per-`(project, environment)` configuration contract.
 *
 ...
 * default, which is exactly what `POLARIS_BRAZE_*` meant before the cutover. A
 ...
 *
 * What is NOT here: the credential. `{instance, api_key}` lives in
 * `destinations.secret_value` on the destination row, because it belongs to
 * one instance rather than to the project, and because it is write-only
 * through every Polaris surface. Note the interaction with `api_host` below:
 * the credential supplies the `{instance}` slug that the host template
 * substitutes, so the two are read together but stored apart.
 *
...
export const PROJECT_CONFIG_NAMESPACE = "braze";
...
export const projectConfigSchema = z.object({
   ...
   * override still routes each instance to its own Braze cluster. Overridden
   * for staging or test endpoints.
   */
...
export type BrazeProjectConfig = z.infer<typeof projectConfigSchema>;
 ...
 */
export function parseBrazeProjectConfig(
