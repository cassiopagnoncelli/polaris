...
export const PROCESSOR_VERSION = "v3" as const;
...
/**
 * What v3 keys touchpoint chains on: the person.
 *
 * v1 and v2 walked a preference list — customer_id, then anonymous_id,
 * then the SDK's session_id — and keyed on whichever the event carried.
 * That fragments the chain at exactly the moment attribution cares
 * about. A visitor arrives on a paid ad and collects touchpoints under
 * `anonymous_id`; they log in and buy, and the purchase keys on
 * `customer_id`. The chain holding first-touch is stranded under the old
 * key, and the conversion is attributed to whatever touchpoint happened
 * to arrive after the login — usually direct or organic. The paid
 * channel that actually earned the conversion gets nothing.
 *
 * The identity stage answers "who is this?" upstream, so v3 reads
 * `profile.profile_id` and nothing else. No fallback: one would
...
/** Minimal shape v3 reads to find the person. */
 ...
 *
 * `undefined` when the event carries no profile — the runtime drops it,
 * exactly as v1/v2 dropped events carrying none of their three
 * identifiers. An event the identity stage could not place has no chain
 * to join, and inventing one from `session_id` would build a chain that
 * can never merge with the person's real one.
 */
 ...
 * Format: `<project_id>::<environment>::<profile_id>`. Mirrors the
...
export function decideAttribution(input: {
  readonly raw: {
    ...
    /** The person, stamped by the identity stage. `null` drops the event. */
    readonly profile?: ResolvedEventProfile | null | undefined;
