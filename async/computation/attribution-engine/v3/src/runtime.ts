 ...
 *
 * Shape mirrors `async/computation/sessionizer/v1/src/runtime.ts` and
 * `sync/legacy/identity-resolver/v1/src/runtime.ts` so every Polaris
 * processor uses the same per-message contract:
 *
 *   1. Subscribe a `PolarisConsumer` to the `resolved.events` topic
...
export interface AttributionEngineRuntimeDeps {
  ...
  /**
   * Projects currently isolated for `resolved.events`. The consumer
...
export function createRuntime(deps: AttributionEngineRuntimeDeps): AttributionEngineRuntime {
  ...
  async function start(): Promise<void> {
    if (started) return;
    started = true;
    const families = consumerFamiliesFor(STREAM_FAMILY_RESOLVED_EVENTS, isolatedProjects);
    ...
    deps.logger.info(
      ...
      "attribution-engine subscribed to resolved.events",
    );
...
async function getPrior(
  ...
  const primary = resolvePrimaryIdentifier(raw.profile);
