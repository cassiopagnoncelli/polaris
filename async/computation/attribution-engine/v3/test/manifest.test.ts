/**
 * Manifest validation for attribution-engine v2.
 *
 * Loads the on-disk `processor.manifest.yaml` and asserts the field set
 * that every released processor MUST carry. See
 * `docs/development/processor-manifests.md`.
 */

import { dirname, resolve } from "node:path";
import { fileURLToPath } from "node:url";

import { loadProcessorManifest, validateProcessorFixtures } from "@polaris/pipeline";
import { describe, expect, it } from "vitest";

const __filename = fileURLToPath(import.meta.url);
const PROCESSOR_DIR = resolve(dirname(__filename), "..");
const REPO_ROOT = resolve(PROCESSOR_DIR, "..", "..", "..", "..");

describe("attribution-engine v2 manifest", () => {
  const loaded = loadProcessorManifest({
    root: REPO_ROOT,
    name: "attribution-engine",
    version: "v3",
  });

  it("carries the expected identity fields", () => {
    expect(loaded.manifest.name).toBe("attribution-engine");
    expect(loaded.manifest.version).toBe("v3");
    expect(loaded.manifest.owner).toBe("platform-data");
  });

  it("declares release_status = released (P8-006)", () => {
    expect(loaded.manifest.release_status).toBe("released");
  });

  it("declares mode = streaming", () => {
    expect(loaded.manifest.mode).toBe("streaming");
  });

  it("consumes analytics.events and emits attribution.events", () => {
    expect(loaded.manifest.inputs).toEqual([{ family: "resolved.events", schema_versions: "*" }]);
    expect(loaded.manifest.outputs).toEqual([
      { family: "attribution.events", schema_versions: [1] },
    ]);
  });

  it("declares the in-memory touchpoints state store", () => {
    expect(loaded.manifest.state_stores).toContain("postgres:attribution_touchpoint_chains");
  });

  it("declares replay metadata with the documented caveats", () => {
    expect(loaded.manifest.replay?.supported).toBe(true);
    expect(loaded.manifest.replay?.restrictions).toContain("first_touch_replay_caveat");
    expect(loaded.manifest.replay?.restrictions).toContain("last_touch_replay_caveat");
    expect(loaded.manifest.replay_notes).toBeDefined();
    expect((loaded.manifest.replay_notes ?? "").length).toBeGreaterThan(0);
  });

  it("declares at least one golden fixture pair and the files resolve on disk", () => {
    expect(loaded.manifest.fixtures).toBeDefined();
    expect((loaded.manifest.fixtures ?? []).length).toBeGreaterThan(0);
    const validation = validateProcessorFixtures({
      manifestPath: loaded.path,
      manifest: loaded.manifest,
    });
    expect(validation.issues).toEqual([]);
    expect(validation.resolvedPaths.length).toBe((loaded.manifest.fixtures ?? []).length * 2);
  });
  it("declares the attribution window and its unit in the description", () => {
    // The window is the whole reason v2 exists. If the manifest does not
    // state it, the semantic definition of this version is incomplete.
    const d = loaded.manifest.description ?? "";
    expect(d).toMatch(/90 days/);
    expect(d).toMatch(/INACTIVITY GAP/);
  });

  it("names a v3 consumer group, so a cutover inherits neither v1's nor v2's offsets", () => {
    expect(loaded.manifest.defaults?.consumer_group).toBe("polaris-attribution-engine-v3");
  });
});
