...
describe("attribution-engine runtime — drop branches", () => {
  it("drops an event the identity stage could not resolve (no emission)", async () => {
    const { runtime, producer } = buildRuntime();
    await runtime.handler(
      buildPayload(
        encodeEnvelope({
          event_id: "evt_no_id",
          // Full campaign, but no profile: the identity stage could not
          // place this event, so there is no chain for it to join.
          profile_id: null,
          campaign: FULL_CAMPAIGN,
        }),
      ),
...
describe("attribution-engine runtime — identifier isolation", () => {
  it("keeps two different people isolated", async () => {
    const store = new InMemoryTouchpointStore();
    const { runtime, producer } = buildRuntime({ store });
    await runtime.handler(
      buildPayload(
...
describe("expired chain writes through startChain, not set", () => {
  ...
  it("uses startChain when the window resets a chain", async () => {
    ...
    const chain = await store.get("checkout::production::01a00000-0000-7000-8000-00000000f001");
...
describe("attribution-engine v3 derived event ids", () => {
