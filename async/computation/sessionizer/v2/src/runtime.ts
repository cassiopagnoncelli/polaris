 ...
 *
 * Shape mirrors `sync/legacy/identity-resolver/v1/src/runtime.ts` so every
 * Polaris processor uses the same per-message contract:
 *
 *   1. Subscribe a `PolarisConsumer` to the `resolved.events` topic family
...
export interface SessionizerRuntimeDeps {
  ...
  /**
   * Projects currently isolated for `resolved.events`. The consumer subscribes
...
export function createRuntime(deps: SessionizerRuntimeDeps): SessionizerRuntime {
  ...
  async function start(): Promise<void> {
    ...
    deps.logger.info(
      ...
      "sessionizer subscribed to resolved.events",
    );
