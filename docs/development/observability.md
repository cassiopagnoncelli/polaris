...
| Prometheus shows scrape targets `DOWN` for `polaris-ingester` / `analytics-projector` | The matching Polaris service isn't running on the host, or it bound to a non-default port.            | Start the service per [Getting Started](./getting-started.md), or edit `infra/prometheus/prometheus.yml` and `curl -X POST http://localhost:9090/-/reload`. |
