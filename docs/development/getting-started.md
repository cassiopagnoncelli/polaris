...
| `dbmate`     | recent              | Applies PostgreSQL migrations. The `db:migrate` script invokes it from `libs/persistence/postgres`. |
...
[`libs/runtime/config`](../../libs/runtime/config/)). The smoke runbook
...
| `pnpm verify`            | **The gate.** Every check CI runs on a bare checkout, in CI's order, behind a millisecond check that the tree's injected copies are current. Run it before you push. |
...
[`libs/persistence/clickhouse`](../../libs/persistence/clickhouse/), which
...
| `pnpm build` fails with `TS2307: Cannot find module '@polaris/…'` naming a package you never touched, and `pnpm install --force` reports "Already up to date" | A freshly provisioned tree. [ADR-0008](../adr/0008-inject-workspace-packages-on-deploy.md)'s injection snapshots part of the workspace graph at install time — before anything is built — so the copy the dependent resolves through has no `dist/`. The lockfile IS current, which is why `--force` is honest and useless at once. | `node scripts/sync-injected-workspace-copies.mjs`. It builds the injected sources, drops `node_modules/.pnpm-workspace-state-v1.json`, re-installs, and verifies. pm runs it in every worktree it creates via `.pm/worktree-setup` — but only in the ones it created AFTER that hook learned to, so a longer-lived worktree can still be missing it. `pnpm verify` checks before it builds and names this instead, so a gate running in a stale tree reports the tree rather than a phantom defect in the package the TS2307 happens to name. |
