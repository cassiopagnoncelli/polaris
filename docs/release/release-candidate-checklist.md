...
| **Workspace gate locally** | `pnpm install && pnpm verify` exits 0 on the RC commit. | Release captain | `pnpm verify` exits 0; it chains build, typecheck, lint, format check, `openapi:check`, `config-schemas:check`, `verify:gates` and the tests. | `pnpm verify` is the same gate `ci.yml` runs on a bare checkout, and `lint:gate-parity` fails if the two drift apart. Running it locally is the captain's "have I rebased onto a clean main" check. |
