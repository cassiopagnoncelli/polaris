...
| **Workspace gate locally** | `pnpm install && node scripts/sync-injected-workspace-copies.mjs && pnpm verify` exits 0 on the RC commit. | Release captain | `pnpm verify` exits 0; it chains build, typecheck, lint, format check, `openapi:check`, `config-schemas:check`, `verify:gates` and the tests. | `pnpm verify` is the same gate `ci.yml` runs on a bare checkout, and `lint:gate-parity` fails if the two drift apart. Running it locally is the captain's "have I rebased onto a clean main" check. The sync between the two is what `ci.yml` runs after its own install: under [ADR-0008](../adr/0008-inject-workspace-packages-on-deploy.md) an install snapshots part of the workspace graph before anything is built, and `pnpm verify` refuses to build a tree in that state rather than reporting a `TS2307` against an innocent package. |
...
| **Logging pipeline reviewed** | Captain reads [`docs/operations/logging.md`](../operations/logging.md), confirms the promtail config at [`infra/loki/promtail-config.yaml`](../../infra/loki/promtail-config.yaml) tails the Pino stdout streams, and verifies the four-label cardinality posture (`service`/`env`/`level`/`project_id` only). | Operator-on-call | Loki + promtail come up via the observability compose, services do not depend on them, and no plaintext-payload logging escapes the shared logger's redaction. | Redaction lives in `@polaris/observability-logger`; Loki sees what the logger emits. |

### Security and secrets

| Item | Evidence required | Owner role | Pass criteria | Notes |
| --- | --- | --- | --- | --- |
| **Secrets not stored in repo** | `rg -nE '(password|secret|token|key)\s*=\s*[\"'\''`]?[A-Za-z0-9+/=._-]{8,}' . --hidden -g '!node_modules' -g '!*.lock' -g '!*.md' -g '!docs/release/release-candidate-checklist.md'` returns no plaintext credentials. Manually inspect any hits — substrings like `secret_ref` or `api_key_id` are structural and acceptable; the offending pattern is a literal that looks credential-shaped. The only committed `.env*` template is [`db/postgres/.env.example`](../../db/postgres/.env.example), which holds a local-only password. | Security reviewer | Zero literal credentials in the repo. The `db/postgres/.env.example` template is the only `.env*` file and it documents that real values are never committed. | The grep is intentionally noisy. The reviewer reads every hit, not "approximately zero" hits. |
...
- **Braze v1:** Braze provides no vendor-side event dedupe; the
  ...
  `@polaris/delivery-destinations` is the canonical idempotency guard.
