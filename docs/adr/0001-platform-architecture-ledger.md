---
id: 0001
title: Platform architecture decisions ledger
status: Accepted
date: 2026-05-12
deciders: legacy
supersedes:
...
Polaris is an internal multi-project event infrastructure platform. The first architecture pass made a large number of load-bearing choices — event contract, transport, identity, replay, ClickHouse layout, dedupe, secrets, observability — across a short window of design discussion. Without a durable record, those choices would be re-litigated by every successor (human or agent).

This ADR is the legacy decision ledger from that first pass, transplanted from `docs/adr/0001-architecture-decisions.md` into MADR shape. Decisions are listed individually so they can be cited by number; the architecture handbook under `docs/architecture/` remains the prose source of truth for how each decision plays out in practice.

Maturity tier assumed: SMB (single team, paying-style internal users, production target). No owner inspirations override the platform-shape decisions captured here.

## Decision Drivers

- Decisions must survive personnel and tooling change.
- Semantic platform truth must live in code/files, not in mutable database rows.
- Replayability within the operational retention window is a primary architectural constraint.
- `project_id` is operational scoping, not a security perimeter — internal cross-project access is allowed.
- Each load-bearing choice must be citeable by number from code, pm cards, or chat.

## Considered Options

- ADR ledger (this file) — many decisions per ledger, prose handbook alongside
- One MADR per decision — full per-decision drivers/options/consequences (~60 files for the first pass)
- PR descriptions only — no durable record

## Decision Outcome

Chosen: **ADR ledger transplanted as a single MADR per topic**, preserving the original numbered decision list. The legacy ledger format is retained because the original drivers and rejected alternatives for each individual line were not recorded at the time and would be backfilled-from-memory if split. New decisions made after this ledger lands open their own per-decision MADR (one decision per file, per `AGENT.md`).

The numbered decisions captured by the first architecture pass:
...
An earlier control-plane framing considered a hybrid or PostgreSQL-first model for more semantic configuration. That was narrowed to:
...
- Positive: load-bearing choices are citeable by number across code, pm cards, and PRs.
- Positive: prose handbook (`docs/architecture/`) and the ledger stay in lockstep — the handbook explains, the ledger commits.
- Positive: future per-decision MADRs can supersede individual line items here without rewriting the ledger.
- Negative: the original drivers and rejected alternatives for each numbered line were not recorded at decision time; this ledger preserves outcomes only.
- Follow-up: when a numbered decision is meaningfully revisited, open a per-decision MADR that supersedes the relevant line and link both directions.
- Revisit if: any of the explicit "v1" defaults (retention windows, dedupe windows, replay window, partition counts, RPO/RTO targets) needs to change with observed traffic.
