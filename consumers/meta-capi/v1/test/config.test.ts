 ...
 *
 * @see consumers/meta-capi/v1/src/config.ts
 */
...
const BASE_ENV: Record<string, string> = {
  ...
  POLARIS_REDPANDA_CLIENT_ID: "meta-capi",
...
describe("metaCapiConfigSchema", () => {
  it("loads defaults for the consumer-scoped knobs", () => {
    ...
    expect(config.meta.partitionsConsumedConcurrently).toBe(4);
  ...
  it("honours overrides for every consumer-scoped knob", () => {
    const config = metaCapiConfigSchema().parse({
      ...
      POLARIS_META_CAPI_CONCURRENCY: "8",
    ...
    expect(config.meta.partitionsConsumedConcurrently).toBe(8);
  ...
  it("rejects non-positive concurrency / timeout", () => {
    expect(() =>
      metaCapiConfigSchema().parse({ ...BASE_ENV, POLARIS_META_CAPI_CONCURRENCY: "0" }),
    ).toThrow();
    expect(() =>
